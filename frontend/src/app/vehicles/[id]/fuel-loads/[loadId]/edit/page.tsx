// src/app/vehicles/[id]/fuel-loads/[loadId]/edit/page.tsx
"use client";
...
export default function EditFuelLoadPage({ params }: { params: { id: string; loadId: string } }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(res => {
        const loads: FuelLoad[] = res.data;
        const idx = loads.findIndex((l: FuelLoad) => l.id === Number(loadId));
        const load = idx >= 0 ? loads[idx] : null;
        if (!load) return router.push(`/vehicles/${vehicleId}`);
        setLoads(loads);
        setFormData({
          date: load.date ? load.date.split('T')[0] : "",
          odometer: load.odometer != null ? String(load.odometer) : "",
          kmDriven: String(load.kmDriven),
          fuelType: load.fuelType ?? "NAFTA_COMUN",
          liters: String(load.liters),
          totalPrice: String(load.totalPrice),
          pricePerLiter: String(load.pricePerLiter),
        });
        if (load.imageUrl) setExistingImage(load.imageUrl);
      })
      .catch(err => console.error(err))
      .finally(() => setLoading(false));
  }, [vehicleId, loadId, router]);

  // Odómetro anterior: la última carga (excluyendo esta) con fecha <= a la fecha ingresada.
  const lastOdometer = useMemo(() => {
    const date = formData.date ? new Date(formData.date) : null;
    if (!date) return null;
    const prev = loads
      .filter(l => l.id !== Number(loadId) && new Date(l.date) <= date)
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
    return prev?.odometer != null ? prev.odometer : null;
  }, [loads, loadId, formData.date]);
  ...
  const calc = useMemo(() => {
    ...
    return { computedPerLiter, computedTotal, computedLiters, kmPerLiter, litersPer100km };
  }, [formData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/fuel/${loadId}`, payload, {
      ...
      console.error(err);
      setError("Ocurrió un error al actualizar la carga de combustible.");
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page max-w-2xl">
      ...
      <PageHeader
        icon={<Droplets />}
        title="Editar Carga"
        subtitle="Modifica los datos de este abastecimiento"
        tone="accent"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="input-group">
            ...
            <p className="text-xs text-text-muted mt-1">
              Al cambiar la fecha, el sistema busca la última carga anterior a esa fecha para calcular los km.
            </p>
          </div>
          ...
          <div className="input-group">
            ...
            {existingImage && !imagePreview && (
              <div className="relative inline-block">
                ...
                <img src={`${API_URL.replace('/api', '')}${existingImage}`} alt="Odómetro actual" className="rounded-xl max-h-40 border border-white/10" />
                <button type="button" onClick={() => setExistingImage(null)} className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-danger text-white flex items-center justify-center" aria-label="Quitar imagen">
                  <X size={14} />
                </button>
              </div>
            )}
            {imagePreview ? (
              <div className="relative inline-block">
                ...
                <button type="button" onClick={() => { setImage(null); if (imagePreview) URL.revokeObjectURL(imagePreview); setImagePreview(null); setExistingImage(null); }} className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-danger text-white flex items-center justify-center" aria-label="Quitar imagen">
                  <X size={14} />
                </button>
              </div>
            ) : (
